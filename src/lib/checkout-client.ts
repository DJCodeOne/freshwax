...
export function init() {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (data.transactions && data.transactions.length > 0) {
            const activeTransactions = data.transactions.filter(t =>
              t.expiresAt && new Date(t.expiresAt) > new Date()
            );
            if (activeTransactions.length > 0) {
              creditExpiry = activeTransactions.reduce((earliest, t) => {
  ...
  // Apply credit to order (only when balance covers full order)
  function toggleApplyCredit(apply) {
    const { total } = calculateTotals();
    if (apply && creditBalance >= total && total > 0) {
      appliedCredit = total;
    } else {
      appliedCredit = 0;
    }
  ...
  function updateCreditDisplay() {
    ...
    // Update applied credit row
    const creditRow = document.getElementById('appliedCreditRow');
    if (creditRow) {
      if (appliedCredit > 0) {
        creditRow.style.display = 'flex';
        document.getElementById('appliedCreditAmount').textContent = `-£${appliedCredit.toFixed(2)}`;
      } else {
  ...
  function updatePaymentButtonText(finalTotal) {
    ...
    if (finalTotal === 0) {
      // Free order - hide payment methods, show free order button
  ...
  function renderCheckout() {
      ...
      <form id="checkoutForm">
        ...
        <div style="background: #fff; border: 2px solid #d1d5db; border-radius: 12px; margin-bottom: 1.5rem; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
          ...
          <div style="padding: 1.25rem;">
            ...
            ${creditBalance > 0 && creditBalance >= total && total > 0 ? `
            <div id="creditSection" style="margin-top: 1rem; padding: 1rem 1.25rem; background: linear-gradient(to bottom, rgba(34, 197, 94, 0.1) 0%, rgba(34, 197, 94, 0.05) 100%); border: 2px solid rgba(34, 197, 94, 0.3); border-radius: 10px;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
                ...
                <span style="font-size: 1.25rem; font-weight: 700; color: #22c55e;">£${creditBalance.toFixed(2)}</span>
              </div>
            ...
            <div style="margin-top: 1rem; padding: 0.75rem 1.25rem; background: #f0fdf4; border: 1px solid rgba(34, 197, 94, 0.2); border-radius: 10px;">
              <span style="font-size: 0.875rem; color: #166534;">You have £${creditBalance.toFixed(2)} credit. Credit can be used when it covers the full order amount.</span>
            </div>
            ` : ''}
          </div>
        </div>
        ...
        <div style="background: #fff; border: 2px solid #d1d5db; border-radius: 12px; margin-bottom: 1.5rem; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
          ...
          <div style="padding: 1.25rem;">
            ...
            <!-- Manual Address Fields - Always show if no saved address (required fields can't be hidden) -->
    ...
    if (duplicatePurchases && duplicatePurchases.length > 0) {
      const itemsList = duplicatePurchases.map(d => `<li><strong>${escapeHtml(d.item.name)}</strong> - ${escapeHtml(d.reason)}</li>`).join('');
    ...
    // Attach event listeners for postcode lookup
    document.getElementById('findAddressBtn')?.addEventListener('click', lookupPostcode);
    ...
    document.getElementById('stripeOption')?.addEventListener('click', () => selectPaymentMethod('stripe'));
    document.getElementById('paypalOption')?.addEventListener('click', () => selectPaymentMethod('paypal'));

    // Attach event listener for credit toggle
    document.getElementById('applyCreditCheckbox')?.addEventListener('change', function() {
      toggleApplyCredit(this.checked);
    });
  ...
  async function handleCustomPayPalClick() {
    const customBtn = document.getElementById('customPaypalBtn');
    const errorMsg = document.getElementById('error-message');
  ...
  async function handleSubmit(e) {
    ...
    if (selectedPaymentMethod === 'paypal' && !isFreeOrder) {
      return;
    }
    ...
    window.trackBeginCheckout?.(cart, total);
    ...
    const orderData = {
      ...
      items: cart.filter(item => item && item.name).map(item => ({
        id: item.id || item.productId,
        productId: item.productId || item.id,
        releaseId: item.releaseId || item.productId || item.id,
        trackId: item.trackId || null,
    ...
    try {
      ...
      if (isFreeOrder) {
        ...
        const freeOrderHeaders = { 'Content-Type': 'application/json' };
        ...
        let response;
        try {
          const stripeHeaders = { 'Content-Type': 'application/json' };
          if (orderData.idToken) {
  ...
  async function initCheckoutWithUser(user, customerId) {
  ...
  // This is more reliable on mobile than attaching handlers to dynamically-created buttons
  // because it catches clicks regardless of when the button was rendered or re-rendered
  document.addEventListener('click', function(e) {
    const target = e.target.closest('#customPaypalBtn');
    if (!target) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  window.addEventListener('pageshow', function(event) {
    if (event.persisted) {
      const submitBtn = document.getElementById('submitBtn');
      const customPaypalBtn = document.getElementById('customPaypalBtn');
      if (submitBtn) {
        submitBtn.disabled = false;
        submitBtn.style.background = '#dc2626';
        submitBtn.style.color = '#fff';
        const { total } = typeof calculateTotals === 'function' ? calculateTotals() : { total: 0 };
        const finalTotal = Math.max(0, total - (appliedCredit || 0));
        if (finalTotal === 0) {
          submitBtn.textContent = 'COMPLETE FREE ORDER';
          submitBtn.style.background = '#16a34a';
        } else {
