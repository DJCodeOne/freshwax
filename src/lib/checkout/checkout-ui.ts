...
// DOM updates, form handling, error display, postcode lookup, rendering
...
export function updateCreditDisplay(state: CheckoutState) {
  const { subtotal, shipping, total } = calculateTotals(state);
  ...
  updatePaymentButtonText(state, finalTotal);
}
...
/**
 * Render the full checkout page HTML.
 * Accepts callbacks for event handlers that are wired up by the orchestrator.
 */
export function renderCheckout(
  ...
  if (duplicatePurchases && duplicatePurchases.length > 0) {
    ...
    document.getElementById('removeDuplicatesBtn')?.addEventListener('click', () => {
      callbacks.removeDuplicatesFromCart(duplicatePurchases);
      document.getElementById('duplicate-warning')?.remove();
      state.duplicatePurchases = [];
      callbacks.renderCheckout();
    });
  }

  // Attach event listeners for postcode lookup
  document.getElementById('findAddressBtn')?.addEventListener('click', callbacks.lookupPostcode);
  ...
  document.getElementById('stripeOption')?.addEventListener('click', () => callbacks.selectPaymentMethod('stripe'));
  document.getElementById('paypalOption')?.addEventListener('click', () => callbacks.selectPaymentMethod('paypal'));

  // Attach event listener for credit toggle
  document.getElementById('applyCreditCheckbox')?.addEventListener('change', function(this: HTMLInputElement) {
