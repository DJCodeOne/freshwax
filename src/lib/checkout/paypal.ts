/**
 * Checkout PayPal — PayPal order creation, approval redirect, SDK rendering.
 */
...
const PAYPAL_BUTTON_SVG = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="pointer-events: none;">
...
export async function handleCustomPayPalClick(state: CheckoutState) {
  const customBtn = document.getElementById('customPaypalBtn') as HTMLButtonElement | null;
  ...
  try {
    ...
    if (!form.checkValidity()) {
      ...
      if (firstInvalid) {
        ...
        setTimeout(() => { try { firstInvalid.focus(); } catch (_e: unknown) { /* intentional: focus may fail if element is not focusable */ } }, 300);
      }
    ...
    customBtn.style.opacity = '1';
    customBtn.innerHTML = `${PAYPAL_BUTTON_SVG} <span style="pointer-events: none;">PAY WITH PAYPAL</span>`;
  }
}
...
export function selectPaymentMethod(state: CheckoutState, method: string) {
  ...
  if (method === 'stripe') {
    if (stripeOption) { stripeOption.style.borderColor = '#dc2626'; stripeOption.style.background = '#fef2f2'; }
    if (paypalOption) { paypalOption.style.borderColor = '#d1d5db'; paypalOption.style.background = '#f9fafb'; }
    ...
    if (paypalOption) { paypalOption.style.borderColor = '#dc2626'; paypalOption.style.background = '#fef2f2'; }
    if (stripeOption) { stripeOption.style.borderColor = '#d1d5db'; stripeOption.style.background = '#f9fafb'; }
    ...
    // PayPal SDK loading is no longer needed - custom button handles clicks directly
  }
}
...
export function resetPayPalButton() {
  ...
  if (customPaypalBtn) {
    (customPaypalBtn as HTMLButtonElement).disabled = false;
    customPaypalBtn.innerHTML = `${PAYPAL_BUTTON_SVG} PAY WITH PAYPAL`;
  }
}
