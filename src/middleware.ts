...
export const onRequest = defineMiddleware(async ({ locals, request }, next) => {
  ...
  if (contentType.includes('text/html')) {
    ...
    // Build CSP — using 'unsafe-inline' for script-src because many components use
    // inline event handlers (onclick, onload, onerror) which cannot use nonces.
    // TODO: Migrate all inline handlers to addEventListener, then re-enable nonce.
