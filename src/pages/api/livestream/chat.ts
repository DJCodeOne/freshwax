...
import { moderateMessage, hmacSha256Hex } from '../../../lib/chat-moderation';
      ...
      const interactiveBotTask = (async () => {
        try {
          ...
          if (isNewUser && shouldWelcomeUser()) {
            ...
            await new Promise(resolve => setTimeout(resolve, TIMEOUTS.BOT_DELAY));
          ...
          // Random chance to comment on a tune (only for non-new users to avoid spam)
            ...
            await new Promise(resolve => setTimeout(resolve, TIMEOUTS.TICK + Math.random() * TIMEOUTS.RATING_DEBOUNCE));
