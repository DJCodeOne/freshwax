...
function showLiveStream(streamData) {
  ...
  try {
    ...
    var relayInfoTitle = streamData.relayNowPlaying || (streamData.relaySource && streamData.relaySource.stationName) || 'Relay Stream';
    ...
    setStreamTicker((streamData.customTitle && streamData.title) ? streamData.title : 'Live and Direct on freshwax.co.uk');
...
async function init() {
  ...
  function updateDjInfoUI(sd) { var relayInfo = sd.relayNowPlaying || (sd.relaySource && sd.relaySource.stationName) || 'Relay Stream'; var name = sd.isRelay ? relayInfo : sd.djName; var controlsDjName = document.getElementById('controlsDjName'); var controlsSetTitle = document.getElementById('controlsSetTitle'); var djInfoBar = document.querySelector('.dj-info-bar'); if (controlsDjName) { if (sd.isRelay) controlsDjName.innerHTML = '<span style="color: #ef4444;">' + escapeHtml(relayInfo) + '</span>'; else controlsDjName.textContent = name || 'DJ'; } if (controlsSetTitle) controlsSetTitle.textContent = sd.isRelay ? relayInfo : (sd.title || 'Live Set'); if (djInfoBar) djInfoBar.classList.add('is-live'); var liveBadge = document.getElementById('liveBadge'); var liveStatusText = document.getElementById('liveStatusText'); var onAirText = document.getElementById('onAirText'); if (liveBadge) { liveBadge.classList.remove('is-loading'); liveBadge.classList.add('is-live'); } if (liveStatusText) liveStatusText.classList.add('hidden'); if (onAirText) onAirText.classList.remove('hidden'); var offOverlay2 = document.getElementById('offlineOverlay'); var initOverlay2 = document.getElementById('initializingOverlay'); if (offOverlay2) { offOverlay2.classList.add('hidden'); offOverlay2.remove(); } if (initOverlay2) initOverlay2.classList.add('hidden'); setReactionButtonsEnabled(true); setChatEnabled(true); }
