// Dump full keys present on Oren's order items so we can see if artistEmail
...
(async () => {
  ...
  const items = parseVal((await r.json()).fields?.items) || [];
  console.log('Total items:', items.length);
  for (const item of items.slice(0, 2)) {
    console.log('\n---', item.name, '---');
    console.log('keys:', Object.keys(item).join(', '));
    console.log('artistEmail:', item.artistEmail);
    console.log('sellerEmail:', item.sellerEmail);
    console.log('stockistEmail:', item.stockistEmail);
  }
