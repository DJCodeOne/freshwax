// List recent pendingCheckouts docs (full dump, newest first)
...
(async () => {
  ...
  const q = { structuredQuery: { from: [{ collectionId: 'pendingCheckouts' }], orderBy: [{ field: { fieldPath: 'createdAt' }, direction: 'DESCENDING' }], limit: 5 } };
  ...
  for (const it of d) {
    ...
    console.log('=== docId:', it.document.name.split('/').pop(), '===');
    console.log(JSON.stringify(f, null, 2));
  }
  if (!n) console.log('No pendingCheckouts docs found (or missing createdAt index/field)');
