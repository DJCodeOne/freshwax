// FreshWax Audio Processor Daemon
// Auto-restarts the audio processor on crash with exponential backoff

const { spawn } = require('child_process');
const fs = require('fs');
const path = require('path');

const SERVER_SCRIPT = path.join(__dirname, 'audio-processor.cjs');
const LOG_DIR = path.join(__dirname, '..', 'logs');
const RESTART_LOG = path.join(LOG_DIR, 'audio-processor-restarts.log');
...
function startServer() {
  ...
  log(`Starting audio processor (attempt #${restartCount})...`);
