...
(async () => {
  ...
  const r = await fetch(FB, { method: 'POST', headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' }, body: JSON.stringify({ structuredQuery: { from: [{ collectionId: 'orders' }], limit: 100 } }) });
  ...
  console.log('Total orders:', arr.length);
  const noCreated = arr.filter(o => !o.createdAt);
  console.log('Orders WITHOUT createdAt:', noCreated.length);
  for (const o of noCreated) {
    const tot = (o.totals && o.totals.total) || o.total || 0;
    console.log('  ' + o.id + ' | orderNumber=' + (o.orderNumber || '?') + ' | total=' + tot + ' | paymentMethod=' + (o.paymentMethod || '?') + ' | status=' + (o.paymentStatus || o.orderStatus || o.status || '?'));
  }
