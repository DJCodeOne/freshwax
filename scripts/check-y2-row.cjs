...
(async () => {
  ...
  const url = 'https://firestore.googleapis.com/v1/projects/' + PROJECT_ID + '/databases/(default)/documents/pendingPayouts/3dlr2YXUjWNVbRiC25me';
  const r = await fetch(url, { headers: { Authorization: 'Bearer ' + token } });
  const j = await r.json();
  const row = Object.fromEntries(Object.entries(j.fields || {}).map(([k, v]) => [k, parseVal(v)]));
  console.log('pendingPayouts/3dlr2YXUjWNVbRiC25me:');
  console.log(JSON.stringify(row, null, 2));

  const url2 = 'https://firestore.googleapis.com/v1/projects/' + PROJECT_ID + '/databases/(default)/documents/orders/fRh0piRRDvBtXaOYOIdD';
  const r2 = await fetch(url2, { headers: { Authorization: 'Bearer ' + token } });
  const j2 = await r2.json();
  const ord = Object.fromEntries(Object.entries(j2.fields || {}).map(([k, v]) => [k, parseVal(v)]));
  console.log('\norders/fRh0piRRDvBtXaOYOIdD:');
  console.log('  orderNumber:', ord.orderNumber);
  console.log('  totals:', JSON.stringify(ord.totals));
  console.log('  paymentMethod:', ord.paymentMethod);
  console.log('  feeRebalanced flag on order?:', ord.feeRebalanced);
