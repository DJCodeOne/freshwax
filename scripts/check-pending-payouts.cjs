// scripts/check-pending-payouts.cjs
// Checks all pending payouts in Firebase
...
async function checkPendingPayouts() {
  ...
  console.log('Got token, querying pendingPayouts...');

  // Query all pending payouts with status 'pending'
  const query = {
    structuredQuery: {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        console.log('\nPending Payouts with status=pending:');
        let count = 0;
        for (const result of results) {
          if (result.document) {
            ...
            console.log('Order ID:', fields.orderId?.stringValue);
            console.log('Order Number:', fields.orderNumber?.stringValue);
            console.log('Artist Name:', fields.artistName?.stringValue);
            console.log('Amount:', fields.amount?.doubleValue);
            console.log('Status:', fields.status?.stringValue);
          }
        }
        if (count === 0) {
          console.log('No pending payouts found with status=pending');
        }
        resolve();
      });
    });
...
checkPendingPayouts().catch(e => console.error('Error:', e.message));
