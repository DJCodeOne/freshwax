...
// ============ DJ-LOBBY.CSS DEAD CLASS ANALYSIS ============
  ...
  while (i < lines.length) {
    if (isRuleDead(lines[i]) && lines[i].includes('{')) {
      ...
      for (let j = i; j < lines.length; j++) {
        ...
        if (depth <= 0) {
          removable.push([start, j]); // 0-indexed inclusive
          i = j + 1;
          break;
        }
      }
...
const djBlocks = findDeadBlocks(djCss, deadDjLobby);
let djDeadLines = 0;
for (const [s, e] of djBlocks) djDeadLines += e - s + 1;
console.log('\n=== DJ-LOBBY DEAD BLOCKS: ' + djBlocks.length + ' blocks, ' + djDeadLines + ' lines ===');
for (const [s, e] of djBlocks) {
  const djLines = djCss.split('\n');
  console.log((s+1) + '-' + (e+1) + ' (' + (e-s+1) + '): ' + djLines[s].trim().substring(0, 80));
}

// Process LIVE
const liveCss = fs.readFileSync('src/styles/live.css', 'utf8');
const liveBlocks = findDeadBlocks(liveCss, deadLive);
let liveDeadLines = 0;
for (const [s, e] of liveBlocks) liveDeadLines += e - s + 1;
console.log('\n=== LIVE DEAD BLOCKS: ' + liveBlocks.length + ' blocks, ' + liveDeadLines + ' lines ===');
const liveLines = liveCss.split('\n');
for (const [s, e] of liveBlocks) {
  console.log((s+1) + '-' + (e+1) + ' (' + (e-s+1) + '): ' + liveLines[s].trim().substring(0, 80));
}
