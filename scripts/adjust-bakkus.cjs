...
(async () => {
  ...
  const ID = 'CCMDrCWRkUXiPN7O83iSfm1BxGo1';
  const r = await fetch('https://firestore.googleapis.com/v1/projects/' + PROJECT_ID + '/databases/(default)/documents/artists/' + ID, { headers: { Authorization: 'Bearer ' + token } });
  const d = await r.json();
  const f = d.fields?.pendingBalance;
  const cur = parseFloat(f?.doubleValue ?? f?.integerValue ?? 0);
  const newBal = cur - 3.5718750000000004;
  console.log('Bakkus pendingBalance: £' + cur.toFixed(4) + ' -> £' + newBal.toFixed(4));
  const p = await fetch('https://firestore.googleapis.com/v1/projects/' + PROJECT_ID + '/databases/(default)/documents/artists/' + ID + '?updateMask.fieldPaths=pendingBalance&updateMask.fieldPaths=updatedAt', {
    method: 'PATCH',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ fields: { pendingBalance: { doubleValue: newBal }, updatedAt: { timestampValue: new Date().toISOString() } } }),
  });
