...
if (!NEEDLE) { console.error('usage: node scripts/find-artist-by-name.cjs <substring>'); process.exit(1); }
...
(async () => {
  ...
  for (const col of ['artists', 'users']) {
    ...
    const q = { structuredQuery: { from: [{ collectionId: col }], limit: 500 } };
    ...
    for (const row of arr) {
      ...
      const fields = ['artistName', 'name', 'displayName', 'email'].map((k) => parseVal(f[k]) || '').join(' ').toLowerCase();
      if (fields.includes(NEEDLE)) {
        console.log(`[${col}] ${id}`);
        for (const k of ['artistName', 'name', 'displayName', 'email', 'isAdmin']) {
