// Print the artists/users record for an artistId so we can verify it's
// the label account rather than an individual contributor.
...
if (!ID) { console.error('usage: node scripts/check-artist.cjs <artistId>'); process.exit(1); }
...
(async () => {
  ...
  for (const col of ['artists', 'users']) {
    const url = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/${col}/${ID}`;
    const res = await fetch(url, { headers: { 'Authorization': `Bearer ${token}` } });
    if (!res.ok) { console.log(`[${col}] not found`); continue; }
    const f = (await res.json()).fields || {};
    console.log(`\n=== ${col}/${ID} ===`);
    for (const k of ['artistName', 'name', 'displayName', 'email', 'labelName', 'isLabel', 'accountType', 'pendingBalance', 'totalEarnings', 'roles']) {
