...
const ORDER_ID = process.argv[2] || 'SwRRRkCavbWH4zlLuRsL';
...
(async () => {
  ...
  const r = await fetch(`https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/orders/${ORDER_ID}`, { headers: { Authorization: 'Bearer ' + token } });
  const f = (await r.json()).fields || {};
  console.log('Order:', ORDER_ID);
  for (const k of ['paymentMethod', 'paymentProvider', 'stripePaymentIntentId', 'stripeSessionId', 'paypalOrderId', 'paypalCaptureId', 'paymentId', 'status', 'totals']) {
    const v = parseVal(f[k]);
    if (v !== null) console.log(`  ${k}:`, typeof v === 'object' ? JSON.stringify(v) : v);
  }
  console.log('  All keys:', Object.keys(f).filter((k) => /pay|stripe|paypal|method|provider/i.test(k)).join(', '));
